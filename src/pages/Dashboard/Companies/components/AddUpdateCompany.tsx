...
import SharedSwitch from "shared/ui/Switch";
...
const AddUpdateCompany = () => {
  ...
  const onSubmit = async (data: IFormValues) => {
    if (!id) {
      const { meta } = await dispatch(addCompany(data));
      ...
      const { meta } = await dispatch(updateCompany(reqData));
      if (meta.requestStatus !== ERequestStatus.FULFILLED) {
        return;
      }
      toast.success("Updated Successfully");
    }
    navigate(`/${EMainRoutes.DASHBOARD}/${ERoutes.COMPANIES}/`);
  };

  const init = useCallback(async () => {
    if (id) {
      const { meta, payload } = await dispatch(getCompanyById(id));
      if (meta.requestStatus !== ERequestStatus.FULFILLED) {
        return;
      }
      reset({
        ...
        positionInView: payload?.positionInView,
    ...
    <FormProvider {...methods}>
      ...
      <Stack spacing={2}>
        ...
        <TextInput
          ...
          rules={requiredRules}
        />
        <SharedSwitch name={"isEnabled"} label={"Is Enabled"} />
        <Box>
